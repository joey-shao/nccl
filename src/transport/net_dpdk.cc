...
NCCL_PARAM(DpdkMaxTasksPerRequest, "DPDK_MAX_TASKS_PER_REQUEST", 8);
NCCL_PARAM(DpdkMinTaskFrames, "DPDK_MIN_TASK_FRAMES", 4096);
NCCL_PARAM(DpdkAckEvery, "DPDK_ACK_EVERY", 8);
NCCL_PARAM(DpdkAckDelayUs, "DPDK_ACK_DELAY_US", 10);
...
static char ncclNetDpdkCtrlIfName[MAX_IF_NAME_SIZE];
...
static std::string ncclNetDpdkDataIpConfPath;
...
static ncclResult_t dpdkInitControlInterface() {
  ...
  strncpy(ncclNetDpdkCtrlIfName, ifName, MAX_IF_NAME_SIZE - 1);
  ncclNetDpdkCtrlIfName[MAX_IF_NAME_SIZE - 1] = '\0';
  memcpy(&ncclNetDpdkCtrlAddr, &ifAddr, sizeof(ifAddr));
  char line[SOCKET_NAME_MAXLEN + 1];
  INFO(NCCL_INIT | NCCL_NET, "NET/DPDK : control plane using %s:%s",
       ncclNetDpdkCtrlIfName, ncclSocketToString(&ncclNetDpdkCtrlAddr, line));
  return ncclSuccess;
}
...
static ncclResult_t dpdkLoadDataIpConfig() {
  ...
  ncclNetDpdkDataIpConfPath = confPath;
...
static ncclResult_t dpdkInitDataDevices() {
  ...
  RTE_ETH_FOREACH_DEV(portId) {
    ...
    if (matched >= (int)ncclNetDpdkDataIps.size()) {
      WARN("NET/DPDK : missing data-plane IPv4 for device %d in %s (need at least %d entries)",
           matched, ncclNetDpdkDataIpConfPath.c_str(), matched + 1);
      return ncclInvalidUsage;
    }
...
ncclResult_t ncclNetDpdkMakeVDevice(int *d, ncclNetVDeviceProps_t *props) {
  ...
  if (devIx >= (int)ncclNetDpdkDataIps.size()) {
    WARN("NET/DPDK : missing data-plane IPv4 for virtual device %d in %s",
         devIx, ncclNetDpdkDataIpConfPath.c_str());
    return ncclInvalidUsage;
  }
