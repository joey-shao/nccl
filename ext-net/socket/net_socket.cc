...
// Reuse plugin-compatible env-param loader.
#define NCCL_PARAM(name, env, defaultVal) NCCL_DPDK_PARAM(name, env, defaultVal)
...
#define SYSCHECK(statement, name)                                              \
  do {                                                                         \
    int _retval = -1;                                                          \
    SYSCHECKSYNC((statement), name, _retval);                                  \
    if (_retval == -1) {                                                       \
...
#define PTHREADCHECK(statement, name)                                          \
  do {                                                                         \
    int _retval = (statement);                                                 \
    if (_retval != 0) {                                                        \
      WARN("Call to %s failed: %s", name, strerror(_retval));                  \
      return ncclSystemError;                                                  \
    }                                                                          \
  } while (0)

static void ncclSetThreadName(pthread_t thread, const char* fmt, ...) {
  char name[16];
  va_list args;
  va_start(args, fmt);
...
socket_send:
  ...
  {
    uint8_t* inlineData = NULL;
    NCCLCHECK(ncclCalloc(&inlineData, MAX_REQUESTS * (SOCKET_CTRL_SIZE + ncclParamSocketInlineSize())));
    comm->inlineData = inlineData;
  }
  *sendComm = comm;
  return ncclSuccess;
}
